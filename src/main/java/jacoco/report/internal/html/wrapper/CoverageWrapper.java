...
public class CoverageWrapper {
    ...
    public static CoverageNodeHighlight wrapNode(ICoverageNode n) {
        CoverageNodeHighlight cnh = new CoverageNodeHighlight(n.getElementType(), n.getName());
        cnh.increment(n);
        return cnh;
    }
    ...
    private static void updateDisplay(IBundleCoverage b) {
        NodeHighlightResults b_nhr;
        if (b instanceof IHighlightNode) {
            b_nhr = ((IHighlightNode) b).getHighlightResults();
            b_nhr.mergeTotaltoBody();
        } else {
            b_nhr = new NodeHighlightResults();
        }
        for (IPackageCoverage p : b.getPackages()) {
            b_nhr.mergeBodyResults(updateDisplay(p));
        }
    }
